...
from evaluatorNew import evaluate
...
data_path = '/content/drive/MyDrive/TGCN/TGCN_with_latest_data/TGCN_with_latest_data/TGC_S&P500/Data'
market_name = 'sp500'
# relation_name = 'sector_industry'  # wikidata or sector_industry # 待修改
parameters = {'seq': 5, 'unit': 64, 'alpha': 0.1}
epochs = 15
# 原始S&P500数据每只股票有2244天，后续由于生成EOD data需要29个pading,处理后的EOD data为2215
valid_index = 1329      # 2215 * 0.6 = 1329
test_index = 1772  # 2215 * 0.2 = 443 2215*0.8=1772
fea_dim = 5
steps = 1

tickers_fname = 'sp500_tickers_extracted.csv'
tickers = np.genfromtxt(os.path.join(data_path,'Relation_Data(478)', tickers_fname), dtype=str, delimiter='\t', skip_header=False)
...
#     os.path.join(data_path, 'Relation_Data(478)',market_name + rname_tail[relation_name])
# )

# test for DTW Adjacent matrix
rel_encoding, rel_mask = load_relation_data('/content/drive/MyDrive/TGCN/TGCN_with_latest_data/TGCN_with_latest_data/TGC_S&P500/Data/Relation_Data(478)/adjacency_matrix_DTW.npy')
...
def validate(start_index, end_index): # 会有 end_index - start_index 个时间点的数据
    """
...
for epoch in range(epochs):
    ...
    with open('../records_error_detection(SP500)_DTW&New_Trading_Strategy(15Epoch,alpha=0.1)_1/valid_model_daily_bt_data{}.json'.format(epoch), 'w') as f:
    ...
    with open('../records_error_detection(SP500)_DTW&New_Trading_Strategy(15Epoch,alpha=0.1)_1/test_daily_bt_data{}.json'.format(epoch), 'w') as f:
...
with open('../records_error_detection(SP500)_DTW&New_Trading_Strategy(15Epoch,alpha=0.1)_1/{}_records_DTW.json'.format(type(model).__name__), 'w') as f:
    f.write(json.dumps(records))
with open('../records_error_detection(SP500)_DTW&New_Trading_Strategy(15Epoch,alpha=0.1)_1/best_model_daily_bt_data_DTW.json', 'w') as f:
    f.write(json.dumps(best_daily_bt_data))
with open('../records_error_detection(SP500)_DTW&New_Trading_Strategy(15Epoch,alpha=0.1)_1/best_test_model_daily_bt_data_DTW.json', 'w') as f:
